<template>
  <q-card class="fillter-bar">
    <q-card-section class="row justify-between items-start full-height">
      <div class="fillter-bar__title col-4 text-left">Property</div>
...
import visitDialog from "../../visits/components/visit-permit-dialog.vue";
...
const selectFilter = (data) => {
  ...
  switch (data.filter.name) {
    case "city":
      handelCheckFilter(selected_filter.value.city, data);
      break;
    case "state":
