<template>
  ...
  <template v-if="create_item_model.units.length !== 0">
    <div class="row" v-for="(unit, i) in create_item_model.units" :key="i">
      ...
      <div class="col-1 q-ma-md">{{ unit.translation[0].name }}</div>
      <div class="col-1 q-ma-md">Model</div>
      ...
      <div class="col-1 q-ma-md">{{ unitForm.sizeMater }}</div>
      <div class="col-1 q-ma-md">
        <q-btn
          flat
...
import unitModel from "./tabs/unit-model.vue";
import unitOwners from "./tabs/owner.vue";
import { create_item_model } from "../create_item_model.js";
...
const editUnit = (index) => {
  // create_item_model.value.units.splice(index, 1);
  unitForm.value = create_item_model.value.units[index];
...
const { getBlockList, getFloorList, propertyType, getVendorsList } =
  propertySetting([
    "getBlockList",
    "getFloorList",
    "propertyType",
